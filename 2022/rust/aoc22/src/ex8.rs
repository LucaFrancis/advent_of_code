...
pub(crate) fn exercise8_1() {
    ...
    let inside = visibility_map.iter().map(|treeline| treeline.iter().sum::<i32>()).sum::<i32>();
...
fn is_visible(trees: Vec<Vec<u32>>, row: usize, column: usize) -> i32 {
    ...
    let top = trees.iter().map(|treeline| treeline[column]).collect::<Vec<u32>>()[0..row].to_vec();
    let bottom = trees.iter().map(|treeline| treeline[column]).collect::<Vec<u32>>()[row + 1..trees.len()].to_vec();
    let left = trees[row][0..column].to_vec();
    let right = trees[row][column + 1..trees.len()].to_vec();
    if vec![top, bottom, right, left].into_iter().any(|direction| direction.iter().all(|tree| tree < &current_tree)) {
        return 1;
    }
    0
}
...
fn get_scenic_score(trees: Vec<Vec<u32>>, row: usize, column: usize) -> i32 {
    ...
    let mut top = trees.iter().map(|treeline| treeline[column]).collect::<Vec<u32>>()[0..row].to_vec();
    top.reverse();
    let bottom = trees.iter().map(|treeline| treeline[column]).collect::<Vec<u32>>()[row + 1..trees.len()].to_vec();
